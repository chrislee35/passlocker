...
class PassLocker:
    ...
    def __init__(
        self,
        password_cb: Callable[[str], bytes],
        dbdir: str | None = None,
    ...
    def _check_master_password(self, checkfile: str, master_password: bytes) -> None:
        ...
        aes_key, hmac_key, _, _ = PassLocker.make_keys(
            master_password, salt=salt, iterations=iterations
        )
    ...
    def _create_master_key_record(self, master_password: bytes) -> dict[str, str]:
        ...
        aes_key, hmac_key, salt, iterations = PassLocker.make_keys(
            master_password, iterations=self.iterations
        )
    ...
    def _initialize_master_password(
        self, checkfile: str, master_password: bytes
    ) -> None:
        if len(master_password) < 20:
            print(
                "Your password is less than 20 characters.    To proceed with this week password, please type: weak password"
            )
            ans: str = input("> ")
        ...
        if self.check_pwnedpasswords(master_password):
            print(
                "This password is listed in pwnedpasswords.com.  Please try a different password."
            )
            raise Exception(
                "This password is listed in pwnedpasswords.com.  Please try a different password."
            )
        ...
        master: dict[str, str] = self._create_master_key_record(
    ...
    def _wrap_b(
        ...
        ..., Any
    ]:  # pyright: ignore[reportExplicitAny, reportUnknownParameterType, reportMissingParameterType]
        def wrapped_b(
            *args, **kwargs
        ) -> (
            Any
        ):  # pyright: ignore[reportExplicitAny, reportUnknownParameterType, reportMissingParameterType, reportAny]
    ...
    def _write_account(self, account: Account, overwrite: bool = True) -> None:
        ...
        if not overwrite and os.path.exists(account_file):
            raise Exception(
                f"Entry for {account.account} ({account.username}) already exists"
            )
    ...
    def _unlink_account(self, account: Account) -> None:
        ...
        if not os.path.exists(account_file):
            raise Exception(
                f"Cannot find entry for {account.account} ({account.username})"
            )
    ...
    @staticmethod
    def make_keys(
        password: bytes, salt: bytes | None = None, iterations: int = 20
    ...
    def change_master_password(self) -> None:
        ...
        aes_key, hmac_key, salt, iterations = PassLocker.make_keys(
            new_password, iterations=self.iterations
        )
    ...
    def list_accounts(self, sep: str = " ") -> list[bytes]:
        ...
        accounts: list[bytes] = [
            x.decode("UTF-8").replace("|", sep, 1).encode("UTF-8") for x in b64decoded
        ]
        return accounts
    ...
    def add_account(
        ...
        acc: Account = Account(account_name, username, created_on, type=acc_type)
        if acc_type == "totp":
    ...
    def add_password(
        self, acc: Account, password: bytes | str, encoding: str = "UTF-8"
    ):
        self._unlock()
        pw: bytes = self._b(password)
    ...
    def get_active_password(self, acc: Account, at_time: int | None = 0):
        ...
        if acc.type == "totp":
            ...
            if ha == "sha1":
                ...
                raise Exception(
                    f"unsupported hash algorithm, {ha}. contact module author"
                )
    ...
    def add_otp_account(
        self, account_name: str, username: str, passwords: list[bytes | str]
    ) -> Account:
        self._unlock()
    ...
    def add_totp_account(
        ...
        num_digits: int = 6,
        hash_algorithm: Literal["sha1", "sha256", "sha512", "md5"] = "sha1",
    ) -> Account:

        self._unlock()
        acc: Account = self.add_account(
            account_name,
            username,
            acc_type="totp",
    ...
    def add_question(
        self, acc: Account, question: str, answer: str, encoding: str = "UTF-8"
    ):
        self._unlock()
