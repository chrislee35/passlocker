...
class PassLockerTest(unittest.TestCase):
    ...
    def get_master_pw(self, prompt: str) -> bytes:  # pyright: ignore[reportUnusedParameter]
        return "mastermastermastermaster098345729834765938".encode('UTF-8')

    def get_pwned_pw(self, prompt: str) -> bytes:  # pyright: ignore[reportUnusedParameter]
        return "passwordpassword123456".encode('UTF-8')

    def test_pwned_password(self):
        with self.assertRaises(Exception):
            pl: PassLocker = PassLocker(self.get_pwned_pw, iterations=10, dbdir='tests/db/') # for testing
            acc: Account = pl.add_account("test", "chris")
            acc.add_password(b"password")
        
    def test_regular_password(self):
        pl: PassLocker = PassLocker(self.get_master_pw, iterations=10, dbdir='tests/db/') # for testing
        acc: Account = pl.add_account("test", "chris")
        acc.add_password(b"password")
        self.assertEqual("password", acc.get_active_password().decode('utf-8'))
        
    def test_otp(self):
        pl = PassLocker(self.get_master_pw, iterations=10, dbdir='tests/db/') # for testing
        acc: Account = pl.add_otp_account("otp_test", "chris", [b"a",b"b",b"c",b"d",b"e",b"f"])
    ...
    def test_totp(self):
        ...
        accounts['sha1'] = pl.add_totp_account("totp_test_sha1", "chris", sha1_key, num_digits=8, time_interval=30, hash_algorithm='sha1')
        accounts['sha256'] = pl.add_totp_account("totp_test_sha256", "chris", sha256_key, num_digits=8, time_interval=30, hash_algorithm='sha256')
        accounts['sha512'] = pl.add_totp_account("totp_test_sha512", "chris", sha512_key, num_digits=8, time_interval=30, hash_algorithm='sha512')
    ...
    def test_utf8(self):
        ...
        pl2: PassLocker = PassLocker(lambda x: "生麦生米生卵生麦生米生卵生麦生米生卵".encode('UTF-8'), iterations=10, dbdir='tests/db/') # for testing
        acc: Account = pl2.add_account("テスト","クリス")
        acc.add_password("マジで？ホンマに？何でやねん？")
        self.assertEqual("マジで？ホンマに？何でやねん？", acc.get_active_password().decode('utf-8'))
